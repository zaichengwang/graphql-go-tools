...
func TestV2Loader_LoadGraphQLResponseDataWithHttpError(t *testing.T) {
	ctrl := gomock.NewController(t)
	productsService := mockedDSWithHttpError(t, ctrl,
		`{"method":"POST","url":"http://products","body":{"query":"query{topProducts{name __typename upc}}"}}`,
		500)
	...
	err = resolvable.storage.PrintNode(resolvable.storage.Nodes[resolvable.storage.RootNode], out)
	assert.NoError(t, err)
	expected := `{"errors":[{"message":"Failed to fetch from Subgraph at path '', '\"HTTP 500: Internal Server Error\"'."}],"data":{},"extensions":{}}`
	assert.Equal(t, expected, out.String())
}

func TestV2Loader_LoadGraphQLResponseDataWithHttpError4XX(t *testing.T) {
	ctrl := gomock.NewController(t)
	productsService := mockedDSWithHttpError(t, ctrl,
		`{"method":"POST","url":"http://products","body":{"query":"query{topProducts{name __typename upc}}"}}`,
		401)
